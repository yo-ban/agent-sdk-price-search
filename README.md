...
- `subscription`: Claude Code にログイン済みのローカル環境（追加設定不要）。provider 強制用の環境変数は SDK 子プロセスへ渡さず、Claude Code の既存ログイン状態を使います
...
API キー不要の検索 discovery は self-hosted SearXNG で実行します。
...
SearXNG は `127.0.0.1:18888` にだけ bind されます。
...
- `workspace_assets/bin/searxng-search`
  - SearXNG 検索 wrapper

### Supporting Files

workspace command が内部で参照する補助ファイルです。agent が直接呼ぶ対象ではありません。
...
workspace にコピーされず、`price-search-core` が Claude Agent SDK に直接登録する tool です。

- `ReadImage`
  - screenshot 画像を Claude に画像 input として渡します
  - `Read` より高解像度の画像 input として渡します

### Agent Behavior Notes

- screenshot を読むときは `Read` ではなく `ReadImage` を使います
- token 使用量を抑えるため、`snapshot` より `eval` / `run-code` を優先します。`snapshot` は click や fill に要素 ref が必要な場合に限定し、取得後は `snapshot-inspect summary/find/controls` で候補を絞ってから参照します
...
結果 JSON は既定で `out/` 配下に保存されます。`--output-file` オプションで保存先を指定できます。Web 上の価格は変動するため、出力結果は実行時点のスナップショットです。
