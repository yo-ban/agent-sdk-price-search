...
def test_get_run_labels_tool_result_with_matching_tool_name(tmp_path: Path) -> None:
    """Tool results reuse the corresponding assistant tool name when available."""
    backend = LocalRunBackend(run_root=tmp_path, python_executable=sys.executable)
    run_directory = tmp_path / "json-label-run"
    _write_inline_run(
        run_directory=run_directory,
        metadata={"run_id": "json-label-run", "product_name": "query", "market": "JP", "currency": "JPY", "max_offers": 1, "started_at": "2026-03-29T00:00:00+00:00", "finished_at": None, "pid": 1, "exit_code": None},
        log_lines=(
            {
                "logged_at": "2026-03-29T00:00:00+00:00",
                "run_id": "json-label-run",
                "event_type": "assistant_message",
                "payload": {
                    "content": [
                        {
    ...
    snapshot = backend.get_run("json-label-run")

    assert snapshot is not None
    assert snapshot.timeline[-1].label == "Tool Result: Bash"
    assert "全自動コーヒーメーカー ABC-1234 価格" in snapshot.timeline[-1].detail


def test_get_run_falls_back_to_generic_tool_result_label(tmp_path: Path) -> None:
    """Tool results without a matching tool_use keep a stable generic title."""
    backend = LocalRunBackend(run_root=tmp_path, python_executable=sys.executable)
    run_directory = tmp_path / "long-label-run"
    long_line = "A" * 200
    _write_inline_run(
        run_directory=run_directory,
        metadata={"run_id": "long-label-run", "product_name": "long", "market": "JP", "currency": "JPY", "max_offers": 1, "started_at": "2026-03-29T00:00:00+00:00", "finished_at": None, "pid": 1, "exit_code": None},
        log_lines=(
            {
                "logged_at": "2026-03-29T00:00:00+00:00",
                "run_id": "long-label-run",
                "event_type": "user_message",
                "payload": {
                    "content": [
                        {
    ...
    snapshot = backend.get_run("long-label-run")

    assert snapshot is not None
    assert snapshot.timeline[0].detail == long_line
    assert snapshot.timeline[0].label == "Tool Result"


def test_get_run_labels_skill_instruction_messages_separately(tmp_path: Path) -> None:
    """Skill bootstrap text is shown as skill instructions instead of plain text."""
    backend = LocalRunBackend(run_root=tmp_path, python_executable=sys.executable)
    run_directory = tmp_path / "skill-instructions-run"
    _write_inline_run(
        run_directory=run_directory,
        metadata={
            "run_id": "skill-instructions-run",
            "product_name": "skill",
            "market": "JP",
            "currency": "JPY",
            "max_offers": 1,
    ...
    snapshot = backend.get_run("skill-instructions-run")

    assert snapshot is not None
    assert snapshot.timeline[0].kind == "system"
    assert snapshot.timeline[0].label == "Skill Instructions: playwright-cli-skill"


def test_get_run_labels_assistant_text_with_stable_titles(tmp_path: Path) -> None:
    """Assistant text uses stable titles instead of repeating body text."""
    backend = LocalRunBackend(run_root=tmp_path, python_executable=sys.executable)
    run_directory = tmp_path / "assistant-text-run"
    _write_inline_run(
        run_directory=run_directory,
        metadata={
            "run_id": "assistant-text-run",
    ...
    snapshot = backend.get_run("assistant-text-run")

    assert snapshot is not None
    assert snapshot.timeline[0].label == "Assistant Message"
    assert snapshot.timeline[1].label == "Summary"
